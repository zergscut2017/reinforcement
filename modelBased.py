...
# Model Network
# Train a model network to predict the observation, reward, done same as real environment
# So we can train the policy network with the model network
...
# gym returns three factor, [observation, reward, done]
...
# Done loss is cross-entropy loss ( softmax )
...
with tf.Session() as sess:
    ...
    while episode_number <= 5000:
        ...
        if done:
            ...
            epx = np.vstack(xs)  # observation relate
            epy = np.vstack(ys)  # action related
            epr = np.vstack(drs)  # reward related
            epd = np.vstack(ds)  # done related
            ...
            if trainTheModel == True:
                ...
                feed_dict = {previous_state: state_prevs, true_observation: state_nexts, true_done: dones,
                             true_reward: rewards}
                # 4 placeholders are defined in model graph, previous_state, true_observation, true_done, true_reward
...
# Compare Here we can examine how well the model is able to approximate the true environment after training.
# The green line indicates the real environment, and the blue indicates model predictions.
